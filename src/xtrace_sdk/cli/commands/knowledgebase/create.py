...
def create_kb(
    ...
    target_api_key = api_key_override or env.get("XTRACE_API_KEY")
    ...
    try:
        ...
        # optionally grant permission for the selected API key
        if perm_num is None:
        ...
        if not api_key_hash:
            console.print("[yellow]Warning:[/] Could not resolve API key hash; permission was not granted.")
            return
        ...
        with _maybe_status(f"Granting [bold]{perm_name}[/] permission…", enable=not json_out):
            try:
                ...
                msg, code = extract_server_error(e)
                console.print(f"[yellow]Warning:[/] grant failed{f' ({code})' if code is not None else ''}: {msg or e}")
                return
